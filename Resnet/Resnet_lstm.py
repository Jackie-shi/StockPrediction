...
class ResnetBlock(Model):
    ...
    def __init__(self, filters, strides=1, residual_path=False):
        ...
        self.c1 = Conv2D(filters, (1, 1), strides=strides, padding='same', use_bias=False)
        self.b1 = BatchNormalization()
        self.a1 = Activation('relu')

        self.c2 = Conv2D(filters, (1, 1), strides=1, padding='same', use_bias=False)
...
class ResNet(Model):
    ...
    def __init__(self, block_list, initial_filters=64):  # block_list表示每个block有几个卷积层
        ...
        self.c1 = Conv2D(self.out_filters, (1, 1), strides=1, padding='same', use_bias=False)
        self.b1 = BatchNormalization()
        self.a1 = Activation('relu')
        self.blocks = tf.keras.models.Sequential()
        #self.s1 = tf.unstack(axis=1)
        self.lstm = tf.keras.layers.LSTM(64, return_sequences=True, activation='relu')
        #self.comb = tf.stack(axis=1)
        # 构建ResNet网络结构
